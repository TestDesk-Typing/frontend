...
const ExamSelect = () => {
  ...
  const [examTypingData, setExamTypingData] = useState({}); // ✅ Stores exam-wise typing data
    ...
    const fetchExams = async () => {
      try {
        ...
        const groupedExams = data.reduce((acc, item) => {
          if (!acc[item.exam]) acc[item.exam] = { exam: item.exam, examNames: [] };
        ...
        if (defaultExam) {
          ...
          fetchAllTypingData(defaultExam.exam);
        }
      } catch (error) {
  ...
  const fetchAllTypingData = async (exam) => {
    try {
      setLoading(true);
      ...
      setExamTypingData(prev => ({
        ...prev,
        [exam]: data
      }));

      setFetchedExams(prev => ({ ...prev, [exam]: true }));
      setLoading(false);
    } catch (error) {
      setLoading(false);
  ...
  const handleExamHover = (examCategory) => {
    ...
    if (!fetchedExams[examCategory.exam]) {
      fetchAllTypingData(examCategory.exam);
    }
  };
  ...
  const countExamNames = () => {
    const counts = {};
    const currentExamData = examTypingData[selectedExamCategory?.exam] || [];
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Select from a variety of typing exams such as SSC CHSL, CGL, DSSSB, EPFO, and RRB. Start your typing test preparation journey with Testdesk today!" />
        <meta name="keywords" content="typing exams, SSC typing, CGL typing, DSSSB typing, EPFO typing, typing test preparation, Testdesk exams" />
        ...
        <meta property="og:description" content="Prepare for typing exams with Testdesk. Practice for SSC CHSL, CGL, DSSSB, EPFO, RRB, and more. Start your typing journey now!" />
        ...
        <meta name="twitter:description" content="Prepare for typing exams with Testdesk. Practice for SSC CHSL, CGL, DSSSB, EPFO, RRB, and more. Start your typing journey now!" />
          ...
          <Row className="mb-4">
            <Col>
              ...
              <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center">
                ...
                <Button variant="outline-primary" className="d-flex align-items-center telegram-btn" onClick={handleRedirect}>
                  <FaTelegram className="me-2" /> Join Telegram
                </Button>
              </div>
            </Col>
          </Row>
                  ...
                  {!loading && selectedExamCategory && (
                    <Row xs={1} sm={2} lg={3} className="g-3">
                      {Array.from(new Set(selectedExamCategory.examNames)).map((examName, idx) => (
                        <Col key={idx}>
                          <Card className="h-100 exam-card-hover" onClick={() => handleExamClick(selectedExamCategory, examName)}>
              ...
              {Array.from(new Set(paragraphs.map(para => para.paper_code))).map((uniqueCode, idx) => {
                ...
                return (
                  <div key={idx} className="col">
                    <Button variant="outline-primary" className="w-100 py-2" onClick={() => handleParagraphClick(para)}>
                      {para.paper_code}
                    </Button>
                  </div>
                );
